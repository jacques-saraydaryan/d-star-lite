import heapq
from utils import stateNameToCoords, render_all, replace_file_content_msg
import threading
...
class d_star_obj:
    ...
    def topKey(self, queue, graph, s_current):
        queue = [(x[1]+self.heuristic_from_s(graph, x[2], s_current), x[1], x[2]) for x in queue]
        queue.sort()
        # print(queue)
        if len(queue) > 0:
            return queue[0][:2]
    ...
    def calculateKey(self, graph, id, s_current, k_m):
        return (min(graph.graph[id].g, graph.graph[id].rhs) + self.heuristic_from_s(graph, id, s_current) , min(graph.graph[id].g, graph.graph[id].rhs))
    ...
    def initDStarLite(self, graph, queue, s_start, s_goal, k_m):
        ...
        heapq.heappush(queue, self.calculateKey(
            graph, s_goal, s_start, k_m) + (s_goal,))
        x = threading.Thread(target=self.computeShortestPath, args=(graph, queue, s_start, k_m,))
        x.start()
    ...
    def print_queue(self, queue):
        x = PrettyTable()
        x.field_names = ["Key 1", "Key 2", "Coordinate"]
        queue_copy = queue.copy()
        queue_copy = sorted(queue_copy, key=operator.itemgetter(0, 1))
        for elt in queue_copy:
            x.add_row(elt)

        print(x)
        replace_file_content_msg(str(x))
